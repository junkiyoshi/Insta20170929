...
void ofApp::draw(){
	...
	ofRotateZ(180);
	ofRotateY(180);
	ofTranslate(ofVec3f(-this->frame_img.getWidth() / 2, -this->frame_img.getHeight() / 2, 0));

	ofColor c;
	for (int y = 0; y < this->depth.rows; y += 4) {
		for (int x = 0; x < this->depth.cols; x += 4) {
			int z = depth.at<unsigned short>(y, x);
			if (z != 0 && z <1024) {
				//ofEllipse(ofVec3f(x, y, z - 1024), 1, 1);
				ofPushMatrix();
				ofTranslate(0, 0, z - 1024);
				ofDrawBitmapString(moji[(int)z / 40], x, y);
				ofPopMatrix();
			}
		}
	}
